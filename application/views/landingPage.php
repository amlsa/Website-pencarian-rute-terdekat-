	...
	'features': [
		{
			...
			'geometry': {
				'type': 'Point',
				'coordinates': [112.524575, -7.884138]
			}
		},
		{
			'type': 'Feature',
		...
		},
		{
			...
			'geometry': {
				'type': 'Point',
				'coordinates': [112.525376, -7.884788]
			}
		}
	]
};
...
function createMarker(){
    datas.forEach(function(index){
		...
		console.log(index.simpulLng);
		console.log(index.simpulLat);
    });
	geojson.features.forEach(function(marker) {
		console.log(marker.geometry.coordinates);
							// [new mapboxgl.Marker({color:"#2c3e50"}).setLngLat(marker.geometry.coordinates).addTo(map).setPopup(new mapboxgl.Popup({ offset: 25 })
							// .setText(index.simpulNama)).addTo(map)];
		new mapboxgl.Marker({
			color:"#4B0082"
...
$("#lokasiAwal").change(function(e){
	...
	console.log(marker[$(this).val()][0]._lngLat.lng);
});

var lineMapLayer;


// coba nambah untuk line
